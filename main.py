...
def main(args):
    ...
    if args.distributed:
        print("INFO: [CUDA] Using distributed training, ")
        if args.remote:
        ...
        print("INFO: [CUDA] Total world size is {}, ngpus_per_node is {}".format(args.world_size, args.ngpus_per_node))
        mp.spawn(main_worker, nprocs=args.ngpus_per_node, args=(args.ngpus_per_node, log_queue, args))
    else:
        # nn.DataParallel (DP)
        print("INFO: [CUDA] Using data parallel training, ")
        if args.dp:
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    print("INFO: [Precision] Training with {} precision.".format(args.precision))
...
def train_epoch(epoch, args, model, train_dataloader, device, optimizer, global_step,
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", total=len(train_dataloader))):
        # if step == 1:
        #     break
