...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            print('INFO: CLIP using Vision Transformer as visual encoder')
        ...
        # phrase prompt configuration (text branch)
        self.pad_token_id = 0
        self.use_phrase_prompt = bool(getattr(args, 'use_phrase_prompt', False))
        self.phrase_prompt_mode = getattr(args, 'phrase_prompt_mode', 'learnable')
        self.phrase_clusters = int(max(0, getattr(args, 'phrase_clusters', 5)))
        self.phrase_temperature = float(max(getattr(args, 'phrase_temperature', 0.2), 1e-6))
        requested_phrase_len = int(getattr(args, 'phrase_prompt_length', self.phrase_clusters or 1))
        self.phrase_prompt_length = self.phrase_clusters if self.phrase_clusters > 0 else 0
        if self.use_phrase_prompt and self.phrase_clusters > 0 and requested_phrase_len != self.phrase_clusters:
            logging.warning("Phrase prompt length (%d) differs from cluster count (%d); using cluster count for residual mapping.",
                            requested_phrase_len, self.phrase_clusters)
        self.phrase_share_layers = bool(getattr(args, 'phrase_share_layers', False))
        self.phrase_residual_scale = float(getattr(args, 'phrase_residual_scale', 1.0))
        self.phrase_kmeans_iters = int(max(1, getattr(args, 'phrase_kmeans_iters', 1)))
        ...
        if self.use_phrase_prompt and self.phrase_clusters > 0:
            proto_layers = 1 if self.phrase_share_layers else self.unified_prompt_layers
            if self.phrase_prompt_mode == 'learnable':
                self.phrase_prototypes = nn.Parameter(
                    torch.randn(proto_layers, self.phrase_clusters, self.transformer_width))
            else:
                self.phrase_prototypes = None

            phrase_mlp_block = nn.Sequential(OrderedDict([
                ("c_fc", nn.Linear(self.transformer_width, self.unified_prompt_width * 4)),
                ("gelu", QuickGELU()),
                ("c_proj", nn.Linear(self.unified_prompt_width * 4, self.transformer_width))
            ]))
            if self.phrase_share_layers:
                self.phrase_mlp = phrase_mlp_block
            else:
                self.phrase_mlp = nn.ModuleList(
                    [copy.deepcopy(phrase_mlp_block) for _ in range(self.unified_prompt_layers)])

            gate_layers = 1 if self.phrase_share_layers else self.unified_prompt_layers
            self.phrase_gate = nn.Parameter(torch.zeros(gate_layers))
        else:
            self.use_phrase_prompt = False
            self.phrase_prototypes = None
            self.phrase_mlp = None
    ...
    def initialize_parameters(self):
        ...
        if self.use_phrase_prompt and isinstance(self.phrase_prototypes, nn.Parameter):
            nn.init.normal_(self.phrase_prototypes, std=0.02)
    ...
    def encode_prompt(self,batch_size,device):
        ...
        print('self.vision_width',self.unified_prompt_width)
        print('transformer_width',self.transformer.width)
    ...
    def incorporate_text_prompt(self, x, prompt_embeddings,text):
        ...
        prompt_token = text.new_zeros((x.size(0), self.text_prompt_length))
        
        # print('self.text_prompt_length',self.text_prompt_length)
        # text_prompt_length = prefix + postfix+text_length =40
    ...
    def _get_phrase_mlp(self, layer_idx: int):
        if self.phrase_mlp is None:
            return None
        if isinstance(self.phrase_mlp, nn.ModuleList):
            return self.phrase_mlp[layer_idx]
        return self.phrase_mlp

    def _get_phrase_prototypes(self, layer_idx: int):
        if self.phrase_prototypes is None:
            return None
        if self.phrase_share_layers:
            return self.phrase_prototypes[0]
        return self.phrase_prototypes[layer_idx]

    def _init_kmeans_centers(self, token_states: torch.Tensor, token_mask: torch.Tensor) -> torch.Tensor:
        batch_size, _, hidden_size = token_states.shape
        centers = []
        for b in range(batch_size):
            valid_idx = torch.nonzero(token_mask[b], as_tuple=False).squeeze(-1)
            if valid_idx.numel() == 0:
                centers.append(token_states.new_zeros(1, self.phrase_clusters, hidden_size))
                continue
            if valid_idx.numel() >= self.phrase_clusters:
                step = max(valid_idx.numel() // self.phrase_clusters, 1)
                chosen = valid_idx[::step][:self.phrase_clusters]
                if chosen.numel() < self.phrase_clusters:
                    chosen = torch.cat([chosen, valid_idx[-1:].repeat(self.phrase_clusters - chosen.numel())], dim=0)
            else:
                chosen = valid_idx
            centers_b = token_states[b:b+1, chosen, :]
            if centers_b.size(1) < self.phrase_clusters:
                pad = self.phrase_clusters - centers_b.size(1)
                pad_tensor = token_states.new_zeros(1, pad, hidden_size)
                centers_b = torch.cat([centers_b, pad_tensor], dim=1)
            centers.append(centers_b)
        return torch.cat(centers, dim=0)

    def _kmeans_phrase(self, token_states: torch.Tensor, token_mask: torch.Tensor):
        centers = self._init_kmeans_centers(token_states, token_mask)
        assign = None
        mask_unsq = token_mask.unsqueeze(-1).float()
        temperature = self.phrase_temperature
        for _ in range(self.phrase_kmeans_iters):
            dist = -torch.cdist(token_states, centers, p=2)
            dist = dist / temperature
            dist = dist.masked_fill(~token_mask.unsqueeze(-1), -1e4)
            assign = torch.softmax(dist, dim=-1)
            assign = assign * mask_unsq
            assign_sum = assign.sum(dim=-1, keepdim=True).clamp(min=1e-6)
            assign = assign / assign_sum
            assign = assign * mask_unsq
            assign_trans = assign.permute(0, 2, 1)
            denom = assign_trans.sum(dim=-1, keepdim=True).clamp(min=1e-6)
            centers = torch.bmm(assign_trans, token_states) / denom
        return assign, centers

    def _phrase_prompt_delta(self, token_states: torch.Tensor, token_mask: torch.Tensor, layer_idx: int):
        if (not self.use_phrase_prompt) or self.phrase_clusters <= 0 or token_states.size(1) == 0:
            return None

        # mask [batch, length-1] 
        # token_states [batch, length-1, dim]
        
        if token_mask is None:
            token_mask = torch.ones(token_states.shape[0], token_states.shape[1], device=token_states.device, dtype=torch.bool)

        if not token_mask.any():
            return None

        original_dtype = token_states.dtype
        tokens_fp32 = token_states.float()
        mask_bool = token_mask.bool()
        mask_unsq = mask_bool.unsqueeze(-1) # [batch, length-1, 1]

        if self.phrase_prompt_mode == 'learnable':
            prototypes = self._get_phrase_prototypes(layer_idx)
            if prototypes is None:
                return None
            # self.phrase_prototypes = nn.Parameter(
            #         torch.randn(proto_layers, self.phrase_clusters, self.transformer_width))
            
            # prototypes [clusters, dim]
            # tokens_fp32 [batch, length-1, dim]
            prototypes = prototypes.to(tokens_fp32.device, dtype=tokens_fp32.dtype)
            proto_norm = F.normalize(prototypes, dim=-1)
            token_norm = F.normalize(tokens_fp32, dim=-1)
            sim = torch.matmul(token_norm, proto_norm.t()) / self.phrase_temperature # [batch, length-1, clusters]
            sim = sim.masked_fill(~mask_unsq, -1e4) # [batch, length-1, clusters]  # [batch, length-1, 1]
            assign = torch.softmax(sim, dim=-1) # [batch, length-1, clusters]
            assign = assign * mask_unsq.float() # [batch, length-1, clusters]
            assign_sum = assign.sum(dim=-1, keepdim=True).clamp(min=1e-6)
            assign = assign / assign_sum
            assign = assign * mask_unsq.float()
            assign_trans = assign.permute(0, 2, 1) # [batch, clusters, length-1]
            denom = assign_trans.sum(dim=-1, keepdim=True).clamp(min=1e-6)
            centers = torch.bmm(assign_trans, tokens_fp32) / denom # [batch, clusters, dim]
        elif self.phrase_prompt_mode == 'kmeans':
            assign, centers = self._kmeans_phrase(tokens_fp32, mask_bool)
        else:
            raise ValueError(f"Unsupported phrase prompt mode: {self.phrase_prompt_mode}")

        phrase_mlp = self._get_phrase_mlp(layer_idx)
        if phrase_mlp is not None:
            centers = centers.view(-1, centers.size(-1))
            centers = phrase_mlp(centers)
            centers = centers.view(assign.size(0), self.phrase_clusters, -1)

        residual = torch.bmm(assign, centers)
        residual = residual * mask_unsq.float()
        residual = residual.to(dtype=original_dtype) * self.phrase_residual_scale
        return residual

    def _get_phrase_gate(self, layer_idx: int, dtype: torch.dtype, device: torch.device):
        if self.phrase_gate is None:
            return None
        if self.phrase_share_layers:
            gate = self.phrase_gate[0]
        else:
            gate = self.phrase_gate[layer_idx]
        return gate.to(device=device, dtype=dtype)

    def _prepare_phrase_delta(self, block: ResidualAttentionBlock, layer_input: torch.Tensor,
                               token_mask: torch.Tensor, text_length: int, layer_idx: int):
        
        # layer_input length, batch, dim 40,32,512
                # token_mask batch, length-1 32,31
                # text_length 32
        if not self.use_phrase_prompt:
            return None

        token_start = self.text_prompt_prefix + 1 # exclude the cls token
        token_end = self.text_prompt_prefix + text_length
        if token_end <= token_start:
            return None

        pre_norm = block.ln_1(layer_input)
        token_slice = pre_norm[token_start:token_end, :, :]
        # length-1, batch, dim
        ...
        mask = token_mask
        if mask is not None:
            mask = mask.to(layer_input.device)

        token_states = token_slice.permute(1, 0, 2) # length-1, batch, dim -> batch, length-1, dim
        # mask batch, length-1 32,31
        # token_states batch, length-1, dim
        residual = self._phrase_prompt_delta(token_states, mask, layer_idx)
        # residual batch, length-1, dim
        if residual is None:
            return None

        residual = residual.permute(1, 0, 2)
        # length-1, batch, dim
        phrase_delta = layer_input.new_zeros(layer_input.shape)
        phrase_delta[token_start:token_end, :, :] = residual

        gate = self._get_phrase_gate(layer_idx, dtype=layer_input.dtype, device=layer_input.device)
        if gate is not None:
            phrase_delta = phrase_delta * gate

        return phrase_delta

    def forward_deep_text_prompt(self, x, text_prompt_embedding, text_length, prompt_token=None):
        
        # text_length 32 
        # x.shape torch.Size([40, 32, 512]) # Length, Batch, Dim
        # prompt_token.shape torch.Size([32, 40]) # Batch, Length
        # text_prompt_embedding  unified_text_prompt.shape torch.Size([12, 32, 8, 512]) # Layers, Batch, Length, Dim
        
        
        hidden_states = x
        num_layers = self.unified_prompt_layers
        token_start = self.text_prompt_prefix + 1
        token_end = self.text_prompt_prefix + text_length
        token_mask = None
        if prompt_token is not None:
            token_mask = prompt_token[:, token_start:token_end].ne(self.pad_token_id)
            
        # print('token_mask',token_mask.shape) # batch, length-1 (exclude the cls token)
        ...
        for i in range(num_layers):
            if i == 0:
                ...
                if i <= len(text_prompt_embedding):
                    ...
                    # print('deep_prompt_emb',deep_prompt_emb.shape) # batch, length, dim
                    deep_prompt_emb = deep_prompt_emb.permute(1, 0, 2)
                    # deep_prompt_emb length, batch, dim
            ...
            phrase_delta = None
            if self.use_phrase_prompt:
                # layer_input length, batch, dim 40,32,512
                # token_mask batch, length-1 32,31
                # text_length 32
                phrase_delta = self._prepare_phrase_delta(block, layer_input, token_mask, text_length, i)
                print('using phrase prompt')
            if hasattr(block, 'set_phrase_delta'):
                block.set_phrase_delta(phrase_delta)

            hidden_states = block((layer_input, self.video_frames, False))[0]
    ...
    def encode_text(self, text,unified_text_prompt, return_hidden=False):
        ...
        prompt_token = prompt_token.to(text.device)
        ...
        # x.shape torch.Size([40, 32, 512]) # Length, Batch, Dim
        # prompt_token.shape torch.Size([32, 40]) # Batch, Length
        # unified_text_prompt.shape torch.Size([12, 32, 8, 512]) # Layers, Batch, Length, Dim
        x = self.forward_deep_text_prompt(x,unified_text_prompt,text_length, prompt_token=prompt_token)
        x = x.permute(1, 0, 2)  # LND -> NLD
