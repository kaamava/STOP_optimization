...
from .losses import CrossEn, RelationJSLoss
...
class CLIP4ClipPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, *inputs, **kwargs):
        ...
        print("INFO: [CUDA] Loading CLIP model:", pretrained_clip_name)
        clip_state_dict = load_clip_state_dict(pretrained_clip_name, pretrained_dir=task_config.pretrained_dir)
        
        
        print("INFO: [CUDA] CLIP4Clip model init from pretrained CLIP:", pretrained_clip_name)
        ...
        print("INFO model.linear_patch:", model.linear_patch)
        ...
        print("INFO model.sim_header:", model.sim_header)
        ...
        if task_config.temperature_new > 1.0:
            print("INFO: [CUDA] Assign new temperature {} to the logit_scale".format(task_config.temperature_new))
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids=None, token_type_ids=None, attention_mask=None, video=None, video_mask=None,
        ...
        if input_ids is not None and video is not None: 
            ...
            print("unified_text_prompt.shape:",unified_text_prompt.shape)
            print("unified_visual_prompt.shape:",unified_visual_prompt.shape)
        ...
        if self.training:
            ...
            # print('visual_output:',visual_output.shape)
            # print('attention_mask:',attention_mask.shape)
            # print('video_mask:',video_mask.shape)
            # raise ValueError
            
            # sequence_output: torch.Size([32, 1, 512])
            # visual_output: torch.Size([32, 1, 512])
            # attention_mask: torch.Size([32, 32])
            # video_mask: torch.Size([32, 12])
            ...
            # inside the training block, just before RelationJSLoss
            
            relation_loss = sequence_output.new_zeros(())
            relation_loss_enabled = bool(getattr(self.task_config, 'use_relation_loss', 0))
            relation_loss_weight = float(getattr(self.task_config, 'relation_loss_weight', 0.0))
            if relation_loss_enabled and relation_loss_weight > 0:
                relation_loss_fct = RelationJSLoss(
                    tau=float(getattr(self.task_config, 'relation_loss_tau', 0.1))
                )

                text_feats = sequence_output.squeeze(1)
                video_feats = visual_output.squeeze(1)

                if torch.distributed.is_available() and torch.distributed.is_initialized():

                    text_feats = text_feats.contiguous()
                    video_feats = video_feats.contiguous()
                    text_feats = all_gather(text_feats)
                    video_feats = all_gather(video_feats)

                relation_loss = relation_loss_fct(text_feats, video_feats)
                loss += relation_loss * relation_loss_weight

            output_dict['relation_loss'] = relation_loss
            # logging.info(f"relation_loss: {relation_loss.item()}")
